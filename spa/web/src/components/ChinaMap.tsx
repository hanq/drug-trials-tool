...
const GEO_URL = "./data/china_map.json";
...
function toEchartsName(name: string): string { return nameMap[name] || name; }

interface Props { provinces: Province[]; onProvinceClick: (name: string) => void }
...
const ChinaMap: React.FC<Props> = ({ provinces, onProvinceClick }) => {
  ...
  const init = useCallback(() => {
    ...
    const max = Math.max(...provinces.map((p) => p.trial_count), 1);
    const data = provinces.map((p) => ({ name: toEchartsName(p.name), value: p.trial_count }));
    chart.current.setOption({
      tooltip: { trigger: "item", formatter: "{b}: {c} 项试验" },
      visualMap: { min: 0, max, text: ["多","少"], inRange: { color: ["#e0f2fe","#0369a1"] }, left: "left", bottom: 20 },
      series: [{
        type: "map", map: "china", roam: true,
        zoom: window.innerWidth < 640 ? 1.3 : window.innerWidth < 1024 ? 1.15 : 1.05,
        label: { show: true, fontSize: Math.min(10, Math.max(7, window.innerWidth / 80)) },
    ...
    chart.current.on("click", (params: any) => {
      const p = provinces.find((pr) => toEchartsName(pr.name) === params.name);
      if (p) onProvinceClick(p.name);
    });
  }, [loaded, provinces, onProvinceClick]);

  useEffect(() => { init(); }, [init]);
  useEffect(() => { const handle = () => chart.current?.resize(); window.addEventListener("resize", handle); return () => window.removeEventListener("resize", handle); }, []);

  return <div ref={ref} style={{ width: "100%", height: Math.min(600, window.innerHeight * 0.7) }} />;
};
