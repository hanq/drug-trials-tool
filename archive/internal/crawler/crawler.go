...
type CrawlResult struct {
	Trial        models.Trial
	Detail       *models.TrialDetail
	Institutions []models.InstitutionRow
...
func ParseTrialDetail(html string, detailID string) *models.TrialDetail {
	detail := &models.TrialDetail{DetailID: detailID}
	...
	for {
		...
		sec := models.DetailSection{Title: title}
		...
		for {
			...
			if stripped != "" {
				sec.Fields = append(sec.Fields, models.DetailField{
...
func parseTable(tableHTML string, sec *models.DetailSection) {
	...
	if hasHeader {
		...
		if len(headers) >= 2 && headers[0] == "序号" {
			...
			sec.Tables = append(sec.Tables, models.DetailTable{
...
func extractInstitutionRows(detail *models.TrialDetail, detailID string) []models.InstitutionRow {
	var result []models.InstitutionRow
	for _, sec := range detail.Sections {
		...
		for _, tb := range sec.Tables {
			...
			for _, row := range tb.Rows {
				ir := models.InstitutionRow{Index: len(result) + 1}
...
func extractMainInvestigator(detail *models.TrialDetail) *models.Investigator {
	for _, sec := range detail.Sections {
		if !strings.Contains(sec.Title, "研究者信息") {
			continue
		}
